...
import { SentIcon, Attachment01Icon, GlobalIcon } from '@hugeicons/core-free-icons'
import { ModelSelector } from './model-selector'
import { ContextUsage } from './context-usage'
import { useState, useMemo, useEffect, useRef, useCallback } from 'react'
import { useQuery } from '@tanstack/react-query'
import { availableModelsQueryOptions, type ModelMetadata } from '@/features/chat/data/queries'
...
export function ChatInput({
  ...
  const [selectedModel, setSelectedModel] = useState<string>(defaultModelId || DEFAULT_MODEL)
  ...
  const tokenCount = useMemo(() => {
    ...
    const allText = messages.map(m => {
      ...
      if (m.parts) return m.parts.filter(p => p.type === 'text').map((p: any) => p.text).join('')
      return ''
    }).join('') + input
