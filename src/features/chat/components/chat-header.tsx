...
export function ChatHeader({ title, isLoading }: ChatHeaderProps) {
  ...
  return (
    <header className="flex items-center justify-between h-14 px-4 border-b border-sidebar-border/50 shrink-0 bg-background/95 backdrop-blur supports-backdrop-filter:bg-background/60">
      <div className="flex items-center gap-3 min-w-0">
        {/* Sidebar toggle - only visible when sidebar is closed */}
        <div className={`transition-all duration-200 ease-out ${isSidebarClosed ? 'w-8 opacity-100' : 'w-0 opacity-0 overflow-hidden'}`}>
          <Button
            variant="ghost"
            size="icon"
