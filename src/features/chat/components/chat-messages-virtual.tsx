...
function messageAnchorId(id: string): string {
  return `message-${id}`
}
...
export function ChatMessagesVirtual({
  ...
  const reasoningSessionsRef = useRef<
  ...
  const selectedReasoningParts = useMemo(
    () =>
      selectedReasoningMessage
        ? getReasoningParts(selectedReasoningMessage)
        : [],
    [selectedReasoningMessage],
  )
  ...
  const selectedReasoningText = useMemo(
    () =>
      selectedReasoningMessage
        ? getReasoningText(selectedReasoningMessage)
        : '',
    [selectedReasoningMessage],
  )
  ...
  const selectedDurationSeconds =
    selectedReasoningSession?.endedAt && selectedReasoningSession.startedAt
      ? Math.max(
          1,
          Math.round(
            (selectedReasoningSession.endedAt -
              selectedReasoningSession.startedAt) /
              1000,
          ),
        )
      : null
      ...
      <div className="w-full max-w-3xl mx-auto p-6">
        {messages.map((message, index) => {
          ...
          const hasNewStreamingContent = isBeingRegenerated
            ? text.length !== regenerationOriginalLengthRef.current ||
              isReasoningStreaming
            : false
          const showRegenerationLoading =
            isBeingRegenerated && !hasNewStreamingContent
          const hideActions =
            isBeingRegenerated ||
          ...
          <DialogFooter>
            <Button
              ...
              disabled={
                shareMode === 'private' ||
                isUpdatingShare ||
                !sharedMessageMap?.has(shareDialogMessage?.id || '')
              }
